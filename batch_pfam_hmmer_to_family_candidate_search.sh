...
python refine_domain_seq.py ${seed}_domain_search.txt ${pro} ${name}.domain.fasta ${evalue1}
### mutiple alignment for hmmer
# muscle3
# muscle -in ${name}.domain.fasta -phyiout ${name}.phyi -clwout ${name}.aln
# muscle5
muscle -align ${name}.domain.fasta -output ${name}.align.fasta
### build Specificity hmmseed
# hmmbuild ${name}.hmm ${name}.phyi
hmmbuild ${name}.hmm ${name}.align.fasta
...
python retrieve.seq.from.all.fasta.py ${name}.id.txt ${pro} ${name}.protein.fasta
# perl retrieve.seq.from.all.fasta.pl ${name}.id.txt ${nuc} ${name}.cds.fasta
python retrieve.seq.from.all.fasta.py ${name}.id.txt ${nuc} ${name}.cds.fasta
